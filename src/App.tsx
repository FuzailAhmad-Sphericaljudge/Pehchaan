...
function NgoInbox({ lang }: { lang: Language }) {
  ...
  React.useEffect(() => { void load(); }, [load]);
  const visible = cases.filter((item) => filter === "all" || (filter === "new" && item.status === "new") || (filter === "high" && (item.priority === "high" || item.type === "debt_bondage")) || (filter === "mine" && Boolean(item.owner))).filter((item) => `${item.id} ${item.workerId} ${item.summary}`.toLowerCase().includes(search.toLowerCase()));
