...
import { ApiError, authApi, CaseDetail, Dashboard, evidenceApi, Language, ngoApi, NgoCase, workerApi } from "./api";
...
function NgoInbox({ lang }: { lang: Language }) {
  const t = ngoText[lang]; const [cases, setCases] = React.useState<NgoCase[]>([]); const [alerts, setAlerts] = React.useState<import("./api").Alert[]>([]); const [filter, setFilter] = React.useState("all"); const [search, setSearch] = React.useState(""); const [loading, setLoading] = React.useState(true); const [error, setError] = React.useState(false);
  const load = React.useCallback(async () => { setLoading(true); try { const [result, alertResult] = await Promise.all([ngoApi.cases(), ngoApi.alerts()]); setCases(result.cases); setAlerts(alertResult.alerts); setError(false); } catch { setError(true); } finally { setLoading(false); } }, []);
