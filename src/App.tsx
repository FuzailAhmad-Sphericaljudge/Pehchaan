...
import { ApiError, authApi, CaseDetail, Dashboard, employerApi, evidenceApi, Language, ngoApi, NgoCase, workerApi } from "./api";
...
function NgoNav({ lang, logout }: { lang: Language; logout: () => void }) {
  ...
  return <aside className="worker-nav ngo-nav"><Link className="brand" to="/ngo">{lang === "hi" ? "पहचान" : "Pehchaan"}<span>.</span></Link><button onClick={() => navigate("/ngo")}>{t.inbox}</button><button onClick={() => navigate("/ngo/analytics")}>{lang === "hi" ? "प्रभाव रिपोर्ट" : "Impact analytics"}</button><button onClick={() => navigate("/ngo/audit")}>{t.audit}</button><button className="logout-link" onClick={logout}>{t.logout}</button></aside>;
}

function NgoInbox({ lang }: { lang: Language }) {
  const t = ngoText[lang]; const [cases, setCases] = React.useState<NgoCase[]>([]); const [alerts, setAlerts] = React.useState<import("./api").Alert[]>([]); const [patterns, setPatterns] = React.useState<{ employer: string; complaintCount: number; independentWorkers: number; caseIds: string[]; signal: string }[]>([]); const [filter, setFilter] = React.useState("all"); const [search, setSearch] = React.useState(""); const [loading, setLoading] = React.useState(true); const [error, setError] = React.useState(false);
...
function EmployerArea({ logout }: { logout: () => void }) { const [data, setData] = React.useState<{ records: import("./api").EmployerRecord[]; compliance: { flagged: number; responded: number; responseRate: number } } | null>(null); const [error, setError] = React.useState(""); const [response, setResponse] = React.useState<Record<string, string>>({}); React.useEffect(() => { employerApi.dashboard().then(setData).catch(() => setError("Could not load employer records.")); }, []); if (error) return <main className="worker-content"><p className="error">{error}</p></main>; if (!data) return <Loading lang="en" />; return <main className="worker-app"><aside className="worker-nav"><Link className="brand" to="/">Pehchaan<span>.</span></Link><strong>Employer portal</strong><button className="logout-link" onClick={logout}>Log out</button></aside><section className="worker-content employer-portal"><p className="eyebrow teal">Responsible employer tools</p><h1>Fair work transparency</h1><div className="stats-grid"><div className="stat-card"><span>Flagged discrepancies</span><strong>{data.compliance.flagged}</strong></div><div className="stat-card"><span>Response rate</span><strong>{data.compliance.responseRate}%</strong></div><div className="stat-card"><span>Worker privacy</span><strong>Protected</strong></div></div><div className="list-panel"><h2>Wage records</h2>{data.records.length ? data.records.map((record) => <div className="list-row" key={record.id}><strong>{record.period}</strong><span>Promised ₹{record.promisedAmount} · Paid ₹{record.paidAmount} · {record.status}</span>{record.status === "disputed" && <><input value={response[record.id] || ""} onChange={(e) => setResponse({ ...response, [record.id]: e.target.value })} placeholder="Your response" /><button className="button button-small" onClick={() => void employerApi.respond(record.id, response[record.id] || "").then(() => setData({ ...data, records: data.records.map((item) => item.id === record.id ? { ...item, status: "responded" } : item) }))}>Respond</button></>}</div>) : <p>No consented wage records yet.</p>}</div><p className="helper">Private complaints, evidence, NGO notes, and other employers' data are never shown here.</p></section></main>; }
