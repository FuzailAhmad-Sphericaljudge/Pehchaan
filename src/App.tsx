...
import { AboutPage, ForOrganizations, ForWorkers, Footer, HowItWorks, JoinPilot, Navbar, PublicHome, SafetyPage } from "./publicSite";
...
// Public marketing pages (Home, HowItWorks, ForWorkers, ForOrganizations, Safety,
// About, Join) and the shared Navbar/Footer now live in ./publicSite.tsx.
...
function ContentPage({ lang, slug }: { lang: Language; slug: string }) {
  ...
  // Phase 35: public version history for legal documents — lets anyone see
  // what a policy said and when it changed, without any admin access.
  const [history, setHistory] = React.useState<{ id: string; locale: string; note: string; publishedAt: string; publishedBy: string }[] | null>(null);
  const [archiveBody, setArchiveBody] = React.useState<string | null>(null);
  const [archiveMeta, setArchiveMeta] = React.useState<{ publishedAt: string } | null>(null);
  ...
  const visibleHistory = history?.filter((item) => item.locale === preferred) || [];
  return <main className="page-hero content-page"><p className="eyebrow">{page?.title || (lang === "hi" ? "जानकारी" : "Information")}</p>{error || (page && !entry) ? <div className="error-box"><p>{lang === "hi" ? "यह पेज अभी उपलब्ध नहीं है।" : "This page is not available yet."}</p>{!error && <p><Link className="text-link" to="/faq">{lang === "hi" ? "सभी पेज देखें →" : "See all pages →"}</Link></p>}</div> : !page ? <Loading lang={lang} /> : <><div className="filter-row">{locales.map((locale) => <button className={locale === preferred ? "filter active" : "filter"} onClick={() => { setViewLocale(locale); setArchiveBody(null); }} key={locale}>{contentLocaleNames[locale] || locale}</button>)}</div>{page.review ? <p className="helper">⚖ {lang === "hi" ? "इस दस्तावेज़ की समीक्षा" : "This document was reviewed and approved by"} {page.review.reviewedBy}{lang === "hi" ? " ने की।" : " for the currently published text."}</p> : page.kind === "legal" ? <p className="error-box">DRAFT — PENDING LEGAL REVIEW. {lang === "hi" ? "यह मसौदा वकील की समीक्षा के लिए है और अंतिम नहीं है।" : "This draft is awaiting lawyer review and is not final."}</p> : null}
          {archiveBody ? <><p className="helper">{lang === "hi" ? "आर्काइव किया गया संस्करण" : "Archived version"}{archiveMeta ? ` — ${new Date(archiveMeta.publishedAt).toLocaleString(lang === "hi" ? "hi-IN" : "en-IN")}` : ""}</p><div className="cms-preview" dangerouslySetInnerHTML={{ __html: tinyMarkdown(archiveBody) }} /><button className="button button-small" onClick={() => setArchiveBody(null)}>{lang === "hi" ? "वर्तमान संस्करण पर लौटें" : "Back to the current version"}</button></> : <><div className="cms-preview" dangerouslySetInnerHTML={{ __html: entry ? tinyMarkdown(entry.body) : "" }} /><p className="helper">{lang === "hi" ? "यह पेज Pehchaan टीम अपडेट करती है — नवीनतम संस्करण हमेशा यहीं दिखता है।" : "This page is maintained by the Pehchaan team — the latest published version always appears here."}</p>{visibleHistory.length ? <div className="list-panel"><h2>{lang === "hi" ? "संस्करण इतिहास" : "Version history"}</h2>{visibleHistory.map((item) => <div className="timeline-item" key={item.id}><strong>{new Date(item.publishedAt).toLocaleString(lang === "hi" ? "hi-IN" : "en-IN")}</strong><span>{item.note || (lang === "hi" ? "कोई नोट नहीं" : "no note")}</span><button className="button button-small" onClick={() => { contentApi.version(slug, item.id).then((result) => { setArchiveBody(result.version.body); setArchiveMeta({ publishedAt: result.version.createdAt }); }).catch(() => setArchiveBody(null)); }}>{lang === "hi" ? "पढ़ें" : "Read"}</button></div>)}</div> : null}</>}</>}</main>;
}
  ...
  return <>
    ...
    <div className="detail-grid cms-grid">
      ...
      {page && selected && <div className="list-panel cms-editor">
        ...
        {page.kind === "legal" && (page.review
          ? <p className="helper">⚖ Reviewed and approved by {page.review.reviewedBy} on {new Date(page.review.reviewedAt).toLocaleDateString()} for the currently published text. Any new publish removes this sign-off until the document is reviewed again.</p>
          : <p className="helper">⚖ DRAFT — pending legal review. After a qualified lawyer approves this text, record the review to remove the public draft label.</p>)}
        {preview
        ...
        <div className="ai-actions">
          ...
          {page.kind === "legal" && !page.review && <button className="button button-small" disabled={busy} onClick={() => { const reviewer = window.prompt("Name of the reviewer who legally approved this document:"); if (!reviewer || !reviewer.trim()) return; setBusy(true); contentApi.recordReview(page.slug, reviewer.trim()).then(() => { setMessage("Legal review recorded — the DRAFT label is removed."); setError(""); return load(); }).catch(() => setError("Could not record the review.")).finally(() => setBusy(false)); }}>Record legal review</button>}
        </div>
        <div aria-live="polite">{message && <p className="success">{message}</p>}{error && <p className="error">{error}</p>}</div>
        ...
        {versions && <div className="cms-versions">{versions.length ? versions.slice(0, 20).map((version) => <div className="timeline-item" key={version.id}><strong>{localeNames[version.locale] || version.locale} · {version.publishedBy}</strong><span>{new Date(version.createdAt).toLocaleString()}{version.note ? ` · ${version.note}` : ""}</span><button className="button button-small" onClick={() => { contentApi.restore(page.slug, version.id).then((result) => { setMessage("Version restored as a new publish."); if (result.version.locale === selected.locale) { setBody(result.version.body); setDirty(false); } return load(); }).catch(() => setError("Could not restore.")); }}>Restore</button></div>) : <p>No versions yet.</p>}</div>}
      </div>}
    </div>
  </>;
}
